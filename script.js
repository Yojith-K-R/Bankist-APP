...
const calculateDMY=function(transDate){

  const date=transDate.getDate();
  const month=transDate.getMonth()+1;
  const year=transDate.getFullYear();
  
  const days=calculateDays(new Date(),transDate);
  console.log(days)

  function calculateDays(date1,date2){
    const days= Math.round(Math.abs(date2-date1)/(24*60*60*1000));
    return days;
    console.log(days);
...
// calculateDays(new Date(2025,2,20),new Date(2025,2,28));

const displayMovements = function (acc, sort = false) {
  containerMovements.innerHTML = '';
  acc.movements=sort?acc.movements.slice(0).sort((a,b)=>a-b):acc.movements;
  acc.movements.forEach(function (mov, i) {
    let transDate=new Date(acc.movementsDates[i]);
    

    let days=calculateDMY(transDate);
...
btnLogin.addEventListener('click', function (e) {
  ...
  const year=transDate.getFullYear();
  const hour=`${transDate.getHours()}`.padStart(2,0);
  const minutes=`${transDate.getMinutes()}`.padStart(2,0);
